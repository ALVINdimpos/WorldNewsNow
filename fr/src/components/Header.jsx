...
import { useGetNotificationsQuery, useMarkAllNotificationsReadMutation, useMarkNotificationReadMutation } from '../store/notificationsApi';

function NotificationBell({ currentUser, goPage }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);

  const { data, refetch } = useGetNotificationsQuery({ limit: 10 }, {
    skip: !currentUser,
  ...
  async function handleClick(n) {
    ...
    setOpen(false);
    if (n.relatedArticle) goPage('article:' + (n.relatedArticle._id || n.relatedArticle));
  }
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        ...
        style={{
          ...
          cursor: 'pointer', fontSize: 16, color: 'var(--muted)', transition: 'all .15s',
        }}
      >
        🔔
        {unread > 0 && (
          <span style={{
            ...
            background: '#F87171', borderRadius: '50%', fontSize: 9,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#fff', fontWeight: 700, fontFamily: "'DM Mono',monospace",
          }}>
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      ...
      {open && (
        <div style={{
          ...
          width: 320, background: 'var(--d3)', border: '1px solid var(--border)',
          borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.55)',
          zIndex: 200, animation: 'modalIn .2s ease', overflow: 'hidden',
        }}>
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '12px 16px', borderBottom: '1px solid var(--border)',
          }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', fontFamily: "'DM Mono',monospace", letterSpacing: 1 }}>
              NOTIFICATIONS
            </span>
            {unread > 0 && (
          ...
          <div style={{ maxHeight: 380, overflowY: 'auto' }}>
            {notifications.length === 0 ? (
              ...
              <div key={n._id} onClick={() => handleClick(n)} style={{
                padding: '12px 16px', borderBottom: '1px solid var(--border)',
                cursor: 'pointer', background: n.read ? 'transparent' : 'rgba(212,168,83,0.04)',
                transition: 'background .15s',
              }}
                onMouseEnter={e => e.currentTarget.style.background = 'var(--d4)'}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0 12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div className="header-live" style={{
              ...
              padding: '5px 12px', borderRadius: 100, fontSize: 11, color: 'var(--green)',
            ...
            <NotificationBell currentUser={currentUser} goPage={goPage} />
            ...
            {currentUser ? (
              <div ref={menuRef} style={{ position: 'relative' }}>
                ...
                {menuOpen && (
                  <div role="menu" style={{
                    position: 'absolute', right: 0, top: 'calc(100% + 10px)',
                    minWidth: 220, background: 'var(--d3)', border: '1px solid var(--border)',
                    borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.55)',
                    padding: '10px 0', zIndex: 200, animation: 'modalIn .2s ease',
                  }}>
                    <div style={{
                      padding: '4px 16px 12px', borderBottom: '1px solid var(--border)', marginBottom: 6,
                    }}>
                      <div style={{
                    ...
                    {currentUser.role === 'journalist' && (
                      <button type="button" role="menuitem" className="btn-gold"
                    ...
                    <button type="button" role="menuitem"
                      onClick={() => { setMenuOpen(false); goPage('bookmarks'); }}
                      style={{
